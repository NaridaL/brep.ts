...
QUnit.assert.doTest = function (face: PlaneFace, brep2: B2, resultEdges: Edge[], resultPoints: V3[], desc?: string) {
	...
	console.log(faceMap)
	this.equal(edges.length, resultEdges.length, resultEdges.length + ' == resultEdges.length == edges.length'+ edges.toSource())
	...
	this.equal(uniquePoints.length, resultPoints.length, resultPoints.length + ' == points.length == resultPoints.length'+ uniquePoints.toSource())
...
QUnit.assert.doTestWithBrep = function (face, faceBrep, brep2, resultEdges, resultPoints, desc, backwards) {
	...
	this.equal(uniquePoints.length, resultPoints.length, resultPoints.length + ' == resultPoints.length == uniquePoints.length'+ uniquePoints.toSource())
	resultPoints.forEach(p => {
...
registerTests({
	...
	'V-shape with spine touching middle of test face'(assert) {
		...
		const brep = B2.tetrahedron(V(0, 0, 0), V(5, 0, 0), V(0, 10, 0), V(0, 10, 6))
	...
	'face box 4'(assert) {
		const box = B2.box(10, 10, 5, 'box').flipped()
		const face = box.faces.find(face => face.surface.plane.normal.z == -1)
		// splitting contour in base position:

		const testBrep = box.translate(3, 3, 0)
		assert.doTestWithBrep(face, box, testBrep, [], [V(3, 10, 5), V(10, 3, 5)],
			'???')
		assert.doTestWithBrep(face, box, testBrep, [
				StraightEdge.throughPoints(V(3, 10, 5), V(3, 3, 5)),
				StraightEdge.throughPoints(V(3, 3, 5), V(10, 3, 5))], [V(3, 10, 5), V(10, 3, 5)],
