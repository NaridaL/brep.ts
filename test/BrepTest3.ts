...
QUnit.assert.doTest = function (face: PlaneFace, brep2: B2, resultEdges: Edge[], resultPoints: V3[], desc?: string) {
...
QUnit.assert.doTestWithBrep = function (face, faceBrep, brep2, resultEdges, resultPoints, desc, backwards) {
	if (brep2 instanceof Face) {
		brep2 = new B2([brep2])
	}
	this.ok(true, `<html><a style='color: #0000ff; text-decoration: underline;'target='blank'
						...
						&edges=[${resultEdges.map(e => e.toSource()).join(',')}]
						&points=[${resultPoints.map(e => e.toSource()).join(',')}]'>${desc}</a>`)
	const faceMap = new Map(), edgeMap = new Map(), likeSurfaceFaces = []
	if (!backwards) {
		brep2.faces.forEach(face2 => {
			face.intersectPlaneFace(face2, faceBrep, brep2, faceMap, edgeMap, likeSurfaceFaces)
		})
	} else {
		brep2.faces.forEach(face2 => {
			face2.intersectPlaneFace(face, brep2, faceBrep, faceMap, edgeMap, likeSurfaceFaces)
		})
	}
	const edges = faceMap.get(face) || [];
	console.log(faceMap)
	...
	this.equal(uniquePoints.length, resultPoints.length, 'uniquePoints.length == resultPoints.length'+ uniquePoints.toSource())
	resultPoints.forEach(p => {
...
registerTests({
	...
	'V-shape with spine touching test face edge middle'(assert) {
		...
		assert.doTest(face, brep.rotateX(PI / 4), [], [])
		assert.doTest(face, brep.rotateX(PI / 4).flipped(), [], [V(0, 0), V(5, 0)]) // StraightEdge.throughPoints(V(0, 0),
		                                                                            // V(5, 0)),
		                                                                            // StraightEdge.throughPoints(V(0, 0),
		                                                                            // V(5, 0))
		assert.doTest(face, brep.rotateX(-PI / 4), [], [V(0, 0), V(5, 0)]) // StraightEdge.throughPoints(V(0, 0), V(5, 0)),
		                                                                   // StraightEdge.throughPoints(V(0, 0), V(5, 0))
		assert.doTest(face, brep.rotateX(-PI / 4).flipped(), [], [])
		assert.doTest(face, brep.rotateX(-PI / 4 * 3), [], [])
		assert.doTest(face, brep.rotateX(-PI / 4 * 3).flipped(), [], [V(0, 0), V(5, 0)]) // StraightEdge.throughPoints(V(0,
		                                                                                 // 0), V(5, 0)),
	...
	'V-shape with spine overlapping test face edge'(assert) {
		...
		assert.doTest(face, brep.rotateX(PI / 4), [], [])
		assert.doTest(face, brep.rotateX(PI / 4).flipped(), [], [V(0, 0), V(3, 0)]) // StraightEdge.throughPoints(V(0, 0),
		                                                                            // V(3, 0)),
		                                                                            // StraightEdge.throughPoints(V(0, 0),
		                                                                            // V(3, 0))
		assert.doTest(face, brep.rotateX(-PI / 4), [], [V(0, 0), V(3, 0)]) // StraightEdge.throughPoints(V(0, 0), V(3, 0)),
		                                                                   // StraightEdge.throughPoints(V(0, 0), V(3, 0))
		assert.doTest(face, brep.rotateX(-PI / 4).flipped(), [], [])
		assert.doTest(face, brep.rotateX(-PI / 4 * 3), [], [])
		assert.doTest(face, brep.rotateX(-PI / 4 * 3).flipped(), [], [V(0, 0), V(3, 0)]) // StraightEdge.throughPoints(V(0,
		                                                                                 // 0), V(3, 0)),
		                                                                                 // StraightEdge.throughPoints(V(0,
		                                                                                 // 0), V(3, 0))
	},
	...
	'tetrahedorn is point on line'(assert) {
		...
		testBrep = B2.tetrahedron(V(4, 0, 10), V(4, 3, 9), V(7, 3, 12), V(1, 3, 12))
		assert.doTestWithBrep(face, box, testBrep, [
		...
		assert.doTestWithBrep(face, box, testBrep.flipped(), [
		...
		assert.doTestWithBrep(face, box, testBrep.rotate(V(4, 0, 10), V3.Z, 90 * DEG), [
				StraightEdge.throughPoints(V(4, 0, 10), V(1, 1, 10)),
				StraightEdge.throughPoints(V(1, 1, 10), V(1, 0, 10))], [V(1, 0, 10), V(4, 0, 10)],
			'face of tetra touches edge of test face, tetra intersects test volume (test face not part of result, points dont really matter)')
	
		assert.doTestWithBrep(face, box, testBrep.rotate(V(4, 0, 10), V3.Z, 90 * DEG).flipped(), [
				StraightEdge.throughPoints(V(1, 1, 10), V(4, 0, 10)),
				StraightEdge.throughPoints(V(1, 0, 10), V(1, 1, 10))], [V(1, 0, 10), V(4, 0, 10)],
			'face of tetra touches edge of test face, tetra intersects test volume (test face not part of result, points dont really matter)')
	
		assert.doTestWithBrep(face, box, testBrep.rotate(V(4, 0, 10), V3.Z, 180 * DEG), [], [],
			'face of tetra touches edge of test face, tetra intersects test volume (test face not part of result, points dont really matter)')
	
		assert.doTestWithBrep(face, box, testBrep.rotate(V(4, 0, 10), V3.Z, 180 * DEG).flipped(), [], [],
			'face of tetra touches edge of test face, tetra intersects test volume (test face not part of result, points dont really matter)')
	},
	...
	'face box'(assert) {
		...
		const testBrep = B2.box(8, 1, 1).translate(0, 0, 9).flipped()
		assert.doTestWithBrep(face, box, testBrep, [
	...
	'face box 3'(assert) {
		const box = B2.box(10, 10, 5, 'box')
		const face = box.faces.find(face => face.surface.plane.normal.like(V3.Z))
		// splitting contour in base position:
	
		const testBrep = B2.box(10, 10, 5, 'box').translate(3, 3, 0)
		assert.doTestWithBrep(face, box, testBrep, [], [],
			'???')
		assert.doTestWithBrep(face, box, testBrep, [
	...
	'assembly'(assert) {
		...
		const extrude = B2.extrudeVertices([V(5, -1), V(2, 2), V(8, 2)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
		...
		const extrude2 = B2.extrudeVertices([V(5, 0), V(2, 3), V(8, 3)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
		...
		const extrude5 = B2.extrudeVertices([V(0, 0), V(3, 2), V(2, 3)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
		...
		const extrude6 = B2.extrudeVertices([V(1, 0), V(3, 0), V(8, 10)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
		...
		const extrude7 = B2.extrudeVertices([V(1, 0), V(3, 0), V(8, 10), V(7, 10)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
	...
	'assembly holes'(assert) {
		...
		const extrude7 = B2.extrudeVertices([V(0, 0), V(5, 8), V(8, 5)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
		const result7 = [PlaneFace.forVertices(P3.XY, [V(0, 0), V(5, 8), V(8, 5), V(0, 0), V(10, 0), V(10, 10), V(0, 10)])];
		assert.doTest2(baseFace, extrude7, result7, 'volume touches inside of test face corner')
	
		const extrude8 = B2.extrudeVertices([V(1, 1), V(1, -1), V(-1, 1)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
		...
		const extrude9 = B2.extrudeVertices([V(-1, -1), V(1, 1), V(1, -1)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
		...
		const extrude10 = B2.extrudeVertices([V(1, 1), V(2, 2), V(2, 1)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
		...
		const extrude11 = B2.extrudeVertices([V(5, 5), V(6, 6), V(6, 5)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
		...
		const extrude12 = B2.extrudeVertices([V(2, 1.5), V(2, 4), V(4.5, 4)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
		...
		const extrude13 = B2.extrudeVertices([V(3, -1), V(4, -1), V(4, 2), V(3, 2)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
		...
		const extrude14 = B2.extrudeVertices([V(1, 1), V(1, 5), V(5, 5)], P3.XY.flipped(), V(0, 0, 10)).translate(0, 0, -2).flipped();
