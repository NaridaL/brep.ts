...
export abstract class XiEtaCurve extends Curve {
	...
	constructor(readonly center: V3, readonly f1: V3, readonly f2: V3, readonly tMin: number, readonly tMax: number) {
		super(tMin, tMax)
...
export function parabola4Projection(
	...
	const wx0 = (x: number) => (Number.isFinite(x) ? snap0(x ** 2 * w2 + x * w1 + wc) : sign(w2) * Infinity)
	if (wx0(tMin) * wx0(tMax) < 0 || extremas.some(x => wx0(x) * (wx0(tMin) + wx0(tMax)) < 0)) {
		console.log(m.str)
		throw new Error(
			'The entire interval must be on one side of the vanishing plane. P=' + P3.vanishingPlane(m).toSource(),
		)
	}
	if (eq0(wc)) {
	...
	}
	{
		...
		if (eq0(delta)) {
			...
			const tMapInv = (t: number) => {
				...
				const xi = 1 - (delta / 2 / wc ** 2) * (Number.isFinite(t) ? t ** 2 / wt : 1 / w2)
				const eta = (t * 2 * wc ** 2 - t ** 2 * delta) / wt / 2 / wc ** 2 - (2 * w1 * wc) / delta
				const xx = acos(xi)
				const p = Number.isFinite(t) ? new V3(t, t ** 2, 0).div(wt) : new V3(0, 1 / w2, 0)
			...
			const tMapInv = (t: number) =>
				sign(t) *
				acosh(1 - (delta / 2 / wc ** 2) * (Number.isFinite(t) ? t ** 2 / (t ** 2 * w2 + t * w1 + wc) : 1 / w2))
