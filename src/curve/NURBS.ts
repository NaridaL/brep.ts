...
*/
	ptDtDdt4(t: number) {
		...
		for (let level = 0; level < degree; level++) {
			if (level == degree - 2) {
				...
				function step(k: int, i: int, dkMinus1iMinus1: Vector, dkMinus1i: Vector) {
	...
	derivate(): NURBS {
		...
		const ps = arrayFromFunction(this.points.length - 1, i =>
			this.points[i].to(this.points[i + 1]).times(k / (this.knots[i + k + 1] - this.knots[i + 1])),
		)
	...
	static bezierKnots(degree: int, tMin: number = 0, tMax: number = 1): number[] {
	...
	static Bezier(points: (V3 | Vector)[], tMin = 0, tMax = 1) {
		return new NURBS(
			points.map(p => (p instanceof V3 ? new Vector(new Float64Array([p.x, p.y, p.z, 1])) : p)),
			points.length - 1,
	...
	static fromHyperbola(hyperbola: HyperbolaCurve, tMin = hyperbola.tMin, tMax = hyperbola.tMax) {
	...
	elevateDegreeBezier() {
		...
		return new NURBS(newPoints, this.degree + 1, newKnots as number[], this.tMin, this.tMax)
	}
	...
	elevateDegree() {
		...
		let result = new NURBS(newPoints, this.degree + 1, newKnots as number[], this.tMin, this.tMax)
...
function minAndMax(arr: ReadonlyArray<number>, start: int = 0, end: int = arr.length) {
	let min = Infinity,
		max = -Infinity
