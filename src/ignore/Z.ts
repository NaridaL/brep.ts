class Complex {
  ...
  times(zOrR: Complex | number): Complex {
    return "number" === typeof zOrR
      ...
      : Z(
          this.re * zOrR.re - this.im * zOrR.im,
          this.im * zOrR.re + this.re * zOrR.im,
        )
  }
  ...
  div(zOrR: Complex | number): Complex {
    ...
    return Z(
      (this.re * z.re + this.im * z.im) / divisor,
      (this.im * z.re - this.re * z.im) / divisor,
    )
  }
  ...
  toString(roundFunction) {
    ...
    return `(${roundFunction(this.re)} ${this.im < 0 ? "-" : "+"} ${Math.abs(
      roundFunction(this.im),
    )} i)`
  }
