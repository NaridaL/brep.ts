import {int, AABB,GOLDEN_RATIO,M4,MINUS,NLA_PRECISION,TAU,Transformable,V3,arrayFromFunction,arrayRange,assert,assertInst,assertVectors,assertf,disableConsole,doubleSignedArea,enableConsole,eq,eq0,ge,gt,isCCW,le,lerp,lt,mapPush,mod,snap} from 'ts3dutils'
import {Mesh, pushQuad} from 'tsgl'

import {Curve, P3, Surface, L3, ParametricSurface, Edge,
    PointVsFace, B2, PlaneSurface} from './index'
                            ...
                            if (!colinearEdges[nextEdgeIndex]) {
                                if (!eq(curveT, isCurve.tMax)) {
                                    ...
                                    if (PointVsFace.INSIDE == pointsToInside) {
                                        ps.push({ p: edge.b, insideDir: isTangent, t: curveT, edge: edge, edgeT: edge.bT, colinear: false})
                                    }
                                }
                                if (!eq(curveT, isCurve.tMin)) {
                                    ...
                                    assert(pointsToInside != PointVsFace.ON_EDGE)
                                    if (PointVsFace.INSIDE == pointsToInside) {
                                        ps.push({ p: edge.b, insideDir: isTangent.negated(), t: curveT, edge: edge, edgeT: edge.bT, colinear: false})
                                    }
                                }
                                ...
                                // on different side ps.push({ p: edge.b, insideDir: insideDir, t: curveT, edge: edge, edgeT: edge.bT, colinear: false}) }
                            }
