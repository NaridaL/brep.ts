...
class Demo extends Component<DemoProps, DemoDesc> {
    constructor(props: DemoProps) {
        super(props)
        this.state = Object.assign({}, props, {canvas: undefined, showingSource: false}) as any
        ...
        return <div {...props} style={{width}}
            ...
            <div className="canvascontainer" ref={r => this.container = r} style={{width: '100%', height}}>
                ...
                {this.props.args.map(arg =>
					...
					<InputComponent type='text' data-name={arg.name} value={arg.value} step={(arg as any).step}
                ...
                <span className='navinfo'>pan: drag-mmb | rotate: drag-rmb | zoom: scroll</span>
                <a className='sourcelink'
                   onClick={this.onClickSource}>{this.state.showingSource ? 'hide source' : 'show source'}</a>
            </div>
            <code className={classnames('src', !demo.showingSource && 'hide')}
                  ref={r => this.sourceContainer = r}>{this.props.f.toSource()}</code>
        </div>
    }
}
...
function setupDemo(canvas: HTMLCanvasElement, demo: DemoDesc, container: HTMLDivElement) {
    //if (demoi !== 0) return
...
const Body = () => <div>
    <h1>BREP.TS</h1>
    <p>This library describes volumes using a boundary representation model. Basically like triangle meshes, except
       faces can be any shape and are not necessarily planar. This allows for fast operations while maintaining a high
       degree of accuracy.</p>

    <p>Once you have two volumes, you can combine them using boolean operations. For instance:</p>
    <h4>box - sphere</h4>
    <Demo id='boxminussphere' width='100%' height='500px' f={function (sphereRadius, sphereZ) {
    ...
    <ul>
        <li>Parametric curves: lines, ellipses, parabolas, hyperbolas, quadratic/cubic beziers, intersection curves
            between parametric and implicit surfaces.
        </li>
    ...
    <Demo id='cyl' width='50%' height='400px' f={B2T.cylinder} args={[
    ...
    <Demo id='text' width='50%' height='400px' f={B2T.text} args={[
    ...
    <Demo id='test1' width='80%' height='400px'
          f={function (/* number */ outsideRadius, /* int */ n, /* number */ insideRadius, /* number */ cornerRadius, /* number */ depth) {
