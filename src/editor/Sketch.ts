...
class Sketch extends Feature {
    ...
    makeCoincident(p1: SegmentEndPoint, p2: SegmentEndPoint) {
        if (p1.coincidence && p2.coincidence) {
            for (let i = 0; i < p2.coincidence.cs.length; i++) {
            ...
            this.constraints.push(p1.coincidence = p2.coincidence = new Constraint('coincident', [p1, p2]))
        }
        p2.x = p1.x
        p2.y = p1.y
    }
    removeFromCoincidence(p: SegmentEndPoint) {
        if (!p.coincidence) return
        p.coincidence.cs.remove(p)
        if (p.coincidence.cs.length == 1) {
            this.constraints.remove(p.coincidence)
            p.coincidence.cs[0].coincidence = null
        }
        p.coincidence = null
    }
    deleteCoincidence(coincidence: Constraint) {
        this.constraints.remove(coincidence)
        coincidence.cs.forEach(p => p.coincidence = null)
    }
    deleteConstraint(constraint: Constraint) {
        if ('coincident' == constraint.type) {
            this.deleteCoincidence(constraint)
        } else {
    ...
    getGroupConstraint(el: any, type: string) {
    ...
    removeFromGroupConstraint(el: any, type: string) {
        const groupConstraint = this.getGroupConstraint(el, type)
        if (!groupConstraint) return
        groupConstraint.cs.remove(el)
        if (1 == groupConstraint.cs.length) {
            this.constraints.remove(groupConstraint)
        }
    ...
    removeFromConstraint(el: SketchSegment | SegmentEndPoint, constraint: Constraint) {
        switch (constraint.type) {
            case 'coincident':
                this.removeFromCoincidence(el)
                break
            case 'parallel':
            case 'colinear':
            case 'equalLength':
                this.removeFromGroupConstraint(el, constraint.type)
                break
            case 'perpendicular':
            case 'pointDistance':
    ...
    apply(publish: (what: string, w: any, feature: Feature) => void, color: GL_COLOR, m4: M4 = M4.IDENTITY, genFeature: Feature = this) {
        const feature = this
    ...
    getLoopForSegment(segment: SketchSegment):Edge[] {
...
class Constraint {
    ...
    constructor(readonly type: string, readonly cs: any[], props?) {
