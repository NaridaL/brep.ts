...
suite('SemiEllipsoidSurface', () => {
	...
	test('loopCCW', assert => {
		const s1 = SemiEllipsoidSurface.UNIT
		const loop = [
			new PCurveEdge(PICurve.forParametricStartEnd(
				new ProjectedCurveSurface(new BezierCurve(V(-0.1040625, -0.095, 1.2), V(-0.1040625, -0.030937500000000007, 1.2), V(-0.065, 0.010000000000000009, 1.2), V(0.047968750000000004, 0.010000000000000009, 1.2), 0, 1), V(0, 0, -1), 0, 1, -Infinity, Infinity),
				new SemiEllipsoidSurface(V3.O, V3.X, V3.Y, V3.Z),
				V(0.7084172378801449, 0.2005401547874497, 0),
				V(1, 0.20120122195537427, 0), 0.01),
