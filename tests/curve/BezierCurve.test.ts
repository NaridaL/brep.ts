...
describe("BezierCurve", () => {
  ...
  test("distanceToPoint", () => {
    ...
    expect(pDist3).toBeLessThan(curve3.at(closestT3 - EPS).distanceTo(p))
    expect(pDist3).toBeLessThan(curve3.at(closestT3 + EPS).distanceTo(p))
  })

  test("isPointsWithBezier()", () => {
    const curve1 = BezierCurve.graphXY(2, -3, -3, 2, -2, 3)
    const curve2 = curve1.transform(M4.rotateLine(V(0.5, 0), V3.Z, PI / 2))
    testCurveISInfos(curve1, curve2, 9)
    ...
    for (const curve of [
      ...
      console.log("Testing " + curve.sce)
      const isInfos = curve.isInfosWithBezier(curve, 0, 1, 0, 1)
      expect(isInfos.length).toBe(1)
  ...
  test("isInfosWithLine", () => {
    ...
    testCurveISInfos(curve, line, 3, "L3(V3.Y, V3.X)")

    const line2 = new L3(V(0, 2, 1), V3.Z)
    testCurveISInfos(curve, line2, 1, "L3(V(0, 2, 1), V3.Z)")

    const line3 = new L3(V3.Z, V3.X)
    testCurveISInfos(curve, line3, 0, "new L3(V3.Z, V3.X)")

    const line4 = new L3(V3.O, V3.Y)
    testCurveISInfos(curve, line4, 1, "L3(V3.O, V3.Y)")
  })
  test("isInfosWithLine 2", () => {
    const curve = new BezierCurve(
  ...
  test("quadratic", () => {
    ...
    expect(b.isQuadratic()).toBeTruthy()
  })
})
