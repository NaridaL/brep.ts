...
describe("BREP", () => {
  ...
  test("BRep.withMergedFaces", () => {
    ...
    const boxToMerge = new BRep(
      box.faces
        .filter((face: PlaneFace) => face.surface.plane.normal1.x != 1)
        .concat(
          box
            .translate(5, 0, 0)
            .faces.filter(
              (face: PlaneFace) => face.surface.plane.normal1.x != -1,
            ),
        ),
      false,
    )

    expect(boxToMerge.faces.length).toEqual(10)
    const boxMerged = boxToMerge.withMergedFaces()
    testBRepOp(boxToMerge, BRep.EMPTY, () => boxMerged, B2T.box(10, 5, 5))
  })
})
