import { Assert, outputLink, suite, test, testCurve, testCurveISInfos, testISTs } from './manager'

import { arrayFromFunction, arraySamples, checkDerivate, DEG, lerp, M4, V, V3, Vector, VV } from 'ts3dutils'
import { BezierCurve, Edge, EllipsoidSurface, HyperbolaCurve, L3, NURBS, P3, XiEtaCurve } from '..'
import { PI, SQRT1_2, SQRT2 } from '../src/math'

suite('NURBS', () => {
	test('openUniformKnots', assert => {
		assert.deepEqual(NURBS.openUniformKnots(7, 3, 0, 4), [0, 0, 0, 0, 1, 2, 3, 4, 4, 4, 4])
	})
	...
	test('EX2D transformed', assert =>
		testCurve(
			...
			NURBS.EX2D.transform(
				M4.forSys(V(0, 0.05, 0), V(-0.05, 0, 0), V(0, 0, 0.05), V(1.3860347309027767, -5.6602156597222235, 0)),
			),
		))
	...
	test('fromBezier', assert => {
		...
		outputLink(assert, {
			drPs: xx.map(x => x[1]),
			drVs: xx.filter(x => isFinite(x[2].x) && !x[2].likeO()).map(x => ({ anchor: x[1], v: x[2] })),
	...
	test('UnitCircle', assert => {
		...
		testCurve(assert, NURBS.UnitCircle(), undefined, 'half circle in 2 segments')
		testCurve(assert, NURBS.UnitCircle(5, -PI, PI), undefined, 'full circle in 5 segments')
		testCurve(assert, NURBS.UnitCircle(3, -PI, PI), undefined, 'full circle in 3 segments')
	})

	test('getSegments 1', assert => {
		const nurbs = NURBS.fromV3s(
			[V(178, 256), V(29, 245), V(12, 159), V(24, 50), V(121, 15), V(247, 11), V(249, 252)],
			4,
		)
		const segments = nurbs.getSegments()
		outputLink(assert, {
			edges: [Edge.forCurveAndTs(nurbs).translate(V3.Z), ...segments.map(s => Edge.forCurveAndTs(s))],
		})
		assert.equal(segments.length, nurbs.points.length - nurbs.degree, 'number of segments')
	...
	test('getSegments on NURBS with duplicate knots', assert => {
		...
		assert.equal(segments.length, nurbs.points.length - nurbs.degree - 1, 'number of segments')
		assert.v3like(segments[0].points[0].p3(), nurbs.points[0].p3())
		assert.v3like(segments.last.points.last.p3(), nurbs.points.last.p3())
		segments.forEach(s => {
			assert.ok(s instanceof NURBS)
	...
	test('getSegments degree=4, multi=3', assert => {
		const nurbs = NURBS.EX2D.withKnot(NURBS.EX2D.knots[5]).withKnot(NURBS.EX2D.knots[5])
		const segments = nurbs.getSegments()
		outputLink(assert, {
			//
			edges: [Edge.forCurveAndTs(nurbs).translate(V3.Z), ...segments.map(s => Edge.forCurveAndTs(s))],
		})
		assert.equal(segments.length, nurbs.points.length - nurbs.degree - 2, 'number of segments')
		assert.v3like(segments[0].points[0].p3(), nurbs.points[0].p3())
		assert.v3like(segments.last.points.last.p3(), nurbs.points.last.p3())
		segments.forEach(s => {
			assert.ok(s instanceof NURBS)
	...
	test('split on new knot', assert => {
		...
		outputLink(assert, {
			...
			edges: [Edge.forCurveAndTs(nurbs).translate(V3.Z), ...[n0, n1].map(s => Edge.forCurveAndTs(s))],
		})
	...
	test('elevateDegree 2', assert => {
		...
		const nurbs = NURBS.fromV3s(
			[V(50, 229), V(13, 186), V(31, 64), V(72, 189), V(133, 191), V(91, 42), V(221, 56), V(203, 227)],
			5,
		)
		const nurbs2 = nurbs.elevateDegree()
		const curves = [nurbs, nurbs2.translate(0, 0, 10)]
	...
	test('removeKnots multi=1', assert => {
		...
		outputLink(assert, {
			edges: [nurbs, nurbs2.translate(0, 0, 10)].map(n => Edge.forCurveAndTs(n)),
		})
	})
	...
	test('isTsWithPlane 1', assert => testISTs(assert, NURBS.EX2D, new P3(V3.XY.unit(), 120), 4))
	test('isTsWithPlane 2', assert =>
		testISTs(assert, NURBS.EX2D, P3.normalOnAnchor(NURBS.EX2D.tangentAt(0.65).cross(V3.Z), NURBS.EX2D.at(0.65)), 2))
	test('isTsWithPlane nurbs contained by plane', assert => testISTs(assert, NURBS.EX2D, P3.XY, 0))
	...
	test('isTsWithPlane EX3D P3.XY', assert => testISTs(assert, NURBS.EX3D, P3.XY, 3))
	test('isTsWithPlane EX3D P3.YZ', assert => testISTs(assert, NURBS.EX3D, P3.YZ, 4))

	suite('isInfosWithLine', () => {
		test('line in curve plane', assert =>
			testCurveISInfos(assert, NURBS.EX2D, new L3(NURBS.EX2D.at(0.65), NURBS.EX2D.tangentAt(0.65).unit()), 2))
		test('parallel to curve plane', assert =>
			testCurveISInfos(
				assert,
				NURBS.EX2D,
				new L3(NURBS.EX2D.at(0.65).plus(V3.Z), NURBS.EX2D.tangentAt(0.65).unit()),
				0,
			))
		...
		test('EX3D, no is', assert => testCurveISInfos(assert, NURBS.EX3D, new L3(V3.O, V(0, -1, 1).unit()), 0))
		test('EX3D, 1 is', assert =>
			testCurveISInfos(assert, NURBS.EX3D, new L3(NURBS.EX3D.at(5), V(0, -1, 1).unit()), 1))
		test('EX3D, 2 is', assert =>
			testCurveISInfos(assert, NURBS.EX3D, L3.throughPoints(NURBS.EX3D.at(-5), NURBS.EX3D.at(10), -100, 200), 2))
	})
	...
	function testSimplify(assert: Assert, c: NURBS) {
		...
		outputLink(assert, {
			edges: [Edge.forCurveAndTs(simple), Edge.forCurveAndTs(c.translate(V3.Z))],
			drPs: arraySamples(0, 1, 8).map(t => c.at(t)),
		})
	}

	test('simplify ellipse', assert => {
		const c = NURBS.fromV3s([V(1, 0, 0), V(1, 1, 0), V(0, 1, 0)], 2, undefined, [1.1, SQRT2 / 3, 1])
		testSimplify(assert, c)
	})

	test('simplify hyperbola', assert => {
		const c = NURBS.fromV3s([V(1, 0, 0), V(1, 1, 0), V(0, 1, 0)], 2, undefined, [1, 2, 1.3])
		testSimplify(assert, c)
	})
	test('simplify hyperbola with w linear', assert => {
		const c = NURBS.fromV3s([V(1, 0, 0), V(1, 1, 0), V(0, 1, 0)], 2, undefined, [1, 2, 3])
		testSimplify(assert, c)
	})
	...
	test('simplify parabola', assert => {
		const c = NURBS.fromV3s([V3.X, V3.O, V3.Y], 2, undefined, [3, 3, 3]).translate(V3.Z)
		testSimplify(assert, c)
	})
	...
	test('simplifyUnit2', assert => {
		...
		const c1 = NURBS.fromV3s([V3.X, V3.O, V3.Y], 2, undefined, [w0, w1, w2]).translate(V3.Z)
