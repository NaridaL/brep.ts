...
// See also https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/Error/Stack
  function extractStacktrace( e, offset ) {
    ...
    if ( e.stack ) {
      stack = e.stack;
      // escape path so it can be used as a RegExp
      if (QUnit.config && QUnit.config.projectPath) {
        let search =  QUnit.config.projectPath.replace(/[.?*+^$[\]\\(){}|-]/g, "\\$&");
        stack = stack.replace(new RegExp(search, 'g'), '');
      }
      stack = stack.split( "\n" );
  ...
  var config = {
    ...
    // Project path. If set, the first occurence on each line of the stack trace
...
// Expose the current QUnit version
  ...
  function Test( settings ) {
    ...
    this.testId = genKey( this.module.name, this.testName );
        ...
        if ( QUnit.objectType( then ) === "function" ) {
          internalStop( test );
            then.call(
              ...
              !config.notrycatch && function (error) {
  ...
  function genKey(module, testName) {
    var str = module && testName ? module + '~' + testName : (module || '') + (testName || '')
    return str
  }
...
// Escape text for attribute or text content.
  function escapeText( s ) {
    ...
    if (s.startsWith("<html>")) return s.substring(6)
  ...
  function escapeText2(s) {
    ...
    s = s.replace(/\s*([^@]*)@([^:]+):(\d+):(\d+)/g,
      ...
      "<td style='color: #008000;padding-right: 1em'>$2</td>" +
      "<td style='color: #EE5757;text-align: right; font-weight: bold;padding-right: 1em'>$3</td>" +
      "<td style='color: #550000;text-align: right'>$4</td></tr>")
    // console.log("SSS", s)
...
// HTML Reporter initialization and load
  ...
  QUnit.log( function( details ) {
    ...
    if ( !details.result && hasOwn.call( details, "expected" ) ) {
      ...
      if ( details.source ) {
        message += "<tr class='test-source'><th>Source: </th><td>" +
          escapeText2( details.source ) + "</td></tr>";
      }
      ...
      message += "<table>" +
        ...
        escapeText2( details.source ) + "</pre></td></tr>" +
        "</table>";
    }
