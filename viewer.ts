...
abstract class BREPGLContext extends LightGLContext {
    shaders: typeof SHADER_TYPE_VAR

    cachedMeshes: WeakMap<any, Mesh>
    ...
    init() {
        ...
        this.shaders = initShaders(viewerGL)
    }
...
let aMeshes: (Mesh & {faceIndexes?: Map<Face, {start: int, count: int}>})[] = [],
        ...
        b2meshes: Mesh[] = []
function viewerPaint(time: int, gl = viewerGL) {
    ...
    drPs.forEach(info => gl.drawPoint(info.p || info, hexIntToGLColor(0xcc0000), 5 / eye.zoomFactor))
    b2planes.forEach(plane => gl.drawPlane(plane, hexIntToGLColor(plane.color), hoverHighlight))

    //console.log(viewerGL.drawCallCount)
}
// let meshes: any = {}
...
function initInfoEvents(paintScreen: () => {}) {
    viewerGL.canvas.addEventListener('mousemove', function (e) {
        const mouseLine = getMouseLine({x: e.clientX, y: e.clientY}, viewerGL)
        ...
        //}
        //if (pp) {
        //	//const pSC = viewerGL.projectionMatrix.times(viewerGL.modelViewMatrix).transformPoint(pp)
...
async function viewerMain() {
    ...
    viewerGL = BREPGLContext.create(create({canvas: document.getElementById('testcanvas') as HTMLCanvasElement}))
    viewerGL.fullscreen()
    viewerGL.canvas.oncontextmenu = () => false

    setupCamera(eye, viewerGL)
    //viewerGL.cullFace(viewerGL.FRONT_AND_BACK);
    viewerGL.clearColor(1.0, 1.0, 1.0, 0.0)
    viewerGL.enable(viewerGL.BLEND)
    viewerGL.enable(viewerGL.DEPTH_TEST)
    viewerGL.enable(viewerGL.CULL_FACE)
    viewerGL.depthFunc(viewerGL.LEQUAL)
    viewerGL.blendFunc(viewerGL.SRC_ALPHA, viewerGL.ONE_MINUS_SRC_ALPHA) // TODO ?!

    viewerGL.clear(viewerGL.COLOR_BUFFER_BIT | viewerGL.DEPTH_BUFFER_BIT)
    viewerGL.loadIdentity()
    viewerGL.scale(10, 10, 10)

    viewerGL.loadIdentity()

    initNavigationEvents(viewerGL, eye, paintScreen)
...
function initNavigationEvents(_gl = viewerGL, eye: {pos: V3, focus: V3, up: V3, zoomFactor: number}, paintScreen: () => void) {
...
function initMeshes(_meshes: { [name: string]: Mesh }, _gl = viewerGL) {
    _gl.makeCurrent()
...
function initShaders(_gl = viewerGL) {
    _gl.makeCurrent()
    return {
...
function setupCamera(_eye: typeof eye, _gl: LightGLContext = viewerGL) {
...
function HJK() {
    ...
    const pcs = (SemiCylinderSurface.UNIT as Surface)
            .rotateZ(-40*DEG)
            .scale(0.5, 0.05, 4)
            .translate(0.5,0,-2)
            .flipped()
